...
function updateSheetQuickPlayButtonAvailability() {
    ...
    const isLoading = hasSelection && !sheetQuickPlayState.isPlaying &&
        (!sheetQuickPlayState.frameReady || sheetQuickPlayState.isStarting || sheetQuickPlayRefreshPending);
    ['sheetQuickPlayButton', 'mobileSheetQuickPlayButton'].forEach(function (buttonId) {
        const buttonEl = document.getElementById(buttonId);
        if (!buttonEl) {
            return;
        }
        buttonEl.disabled = !sheetQuickPlayState.isPlaying && (!hasSelection || isLoading);
        buttonEl.classList.toggle('is-loading', isLoading);
        buttonEl.textContent = isLoading ? '…' : (sheetQuickPlayState.isPlaying ? '■' : '▶');
        buttonEl.title = uiText(isLoading
            ? 'editor.quickPlay.loadingSounds'
            : (sheetQuickPlayState.isPlaying ? 'editor.quickPlay.stop' : 'editor.quickPlay.playSelected'));
    });
...
function positionMobileSheetQuickPlayFrame() {
    ...
    frameEl.classList.toggle('is-mobile-control', sheetQuickPlayState.frameReady && !sheetQuickPlayRefreshPending);
}
...
function selectSheetQuickPlayPattern(patternId) {
    ...
    if (sheetQuickPlayState.isPlaying || sheetQuickPlayState.isStarting) {
        stopSheetQuickPlay();
    }
...
function hasSheetQuickPlayCyclicPickup(pattern) {
    if (typeof pattern.quickPlayHasCyclicPickup === 'boolean') {
        return pattern.quickPlayHasCyclicPickup;
    }
    const notes = getSheetQuickPlayPatternNotes(pattern);
    const pickupEndStep = getSheetQuickPlayPickupEndStep(
        notes,
        getSheetQuickPlayPatternInStep(pattern),
        getReadRhythmConfig().stepsPerBar
    );
    // At the end of a pattern, IN leads back into its cycle instead of discarding it.
    return pickupEndStep > 0 &&
        ((pattern.labelType || pattern.label) === 'Begleitung' || pickupEndStep === notes.length);
}
...
function buildSheetQuickPlayConfiguredSections(preparedPatterns) {
    ...
    groups.forEach(function (group, groupIndex) {
        ...
        const hasCyclicPickup = group.some(function (entry) {
            return hasSheetQuickPlayCyclicPickup(entry.pattern);
        });
        ...
        group.forEach(function (groupEntry) {
            ...
            const keepFullPatternAfterPickup = pickupEndStep > 0 &&
                hasSheetQuickPlayCyclicPickup(pattern);
            let pickupNotes = buildSheetQuickPlayPickupNotes(patternNotes, inStep, stepsPerBar);
            let mainNotes = pickupEndStep > 0 && !keepFullPatternAfterPickup
            ...
            let mainHighlightRefs = pickupEndStep > 0 && !keepFullPatternAfterPickup
                ? patternHighlightRefs.slice(pickupEndStep)
                : patternHighlightRefs.slice();
            let sectionStartStep = pickupEndStep > 0 && !keepFullPatternAfterPickup
                ? pickupEndStep
                : 0;

            if (pickupEndStep > 0 && !hasHostSection && !hasCyclicPickup && group.length === 1) {
            ...
            } else if (pickupEndStep > 0 && !hasHostSection && !hasCyclicPickup) {
            ...
            if ((label === 'Begleitung' || keepFullPatternAfterPickup) && mainNotes.length > 0) {
...
function scheduleSheetQuickPlayPreparation(delay) {
    ...
    sheetQuickPlayState.preparationTimer = window.setTimeout(
        ...
        delay === undefined ? 120 : Math.max(0, Number(delay) || 0)
    );
}
...
function stopSheetQuickPlay(options) {
    ...
    sheetQuickPlayState.isStarting = false;
    clearSheetQuickPlaySchedulerPump();
    const frameEl = document.getElementById('sheetQuickPlayFrame');
...
function startSheetQuickPlay() {
    if (sheetQuickPlayState.isStarting || sheetQuickPlayRefreshPending) {
        return;
    }
    const frameEl = document.getElementById('sheetQuickPlayFrame');
    const signature = getSheetQuickPlayPreparationSignature();
    const startFromParent = frameEl && frameEl.contentWindow
        ...
        : null;
    // Keep AudioContext creation/resume in the actual click, including desktop Safari.
    if (sheetQuickPlayState.frameReady &&
            sheetQuickPlayState.preparedSignature === signature &&
            typeof startFromParent === 'function' &&
            startFromParent()) {
        sheetQuickPlayState.isStarting = true;
        updateSheetQuickPlayButtonAvailability();
        return;
    }
...
function toggleSheetQuickPlay() {
    if (sheetQuickPlayState.isPlaying) {
        stopSheetQuickPlay({ preservePreparedPlayer: true });
        return;
    }
    startSheetQuickPlay();
}
...
function handleEmbeddedAudioPlayerMessage(event) {
    ...
    if (isSheetQuickPlayFrame) {
        if (!sheetQuickPlayState.preparedSignature) {
            return;
        }
        ...
        if (message.type === 'barabeat-audio-state') {
            if (message.state === 'ready') {
                if (sheetQuickPlayState.preparationTimer !== null ||
                        sheetQuickPlayState.preparedSignature !== getSheetQuickPlayPreparationSignature()) {
                    return;
                }
            ...
            if (message.state === 'error') {
                alert(message.message || uiText('player.error.audioStart'));
            }
            return;
        }
        return;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('#sheetQuickPlayTempo').addEventListener('change', function () {
        const nextTempo = getSheetQuickPlayTempo();
        const frameEl = document.getElementById('sheetQuickPlayFrame');
        const bpmControl = frameEl && frameEl.contentDocument
            ? frameEl.contentDocument.querySelector('#bpm')
            : null;
        if (sheetQuickPlayState.frameReady && bpmControl) {
            bpmControl.value = String(nextTempo);
            bpmControl.dispatchEvent(new frameEl.contentWindow.Event('input', { bubbles: true }));
            sheetQuickPlayState.preparedSignature = getSheetQuickPlayPreparationSignature();
        } else {
            scheduleSheetQuickPlayPreparation(0);
        }
